...
public class RushBuyClacServiceImpl implements IRushBuyClacService{
	...
	public void rushBuyCalc(Map<String, Object> data) {
		...
		if ((userlist == null || userlist.isEmpty()) && (orderlist == null || orderlist.isEmpty())) {
			insertNotice("抢购"+cardInfo.get("typename")+"分配失败", data.get("calctime")+"获取参与抢购的人和待出售订单为0：此次会员卡抢购失败", Constants.FAILED);
			return;
		}
		...
		//获取多次结算的每次结算的抢购成功率
		List<Map<String, Object>> ratelist = this.rushBuyClacMapper.getConfigRate(cardInfo);
		...
		for (int i=0; i < indexlist.size(); i++){
			...
			if(i >= orderlist.size()) {
				try {
					 ...
					 * 出售系统会员卡
					 */
					//增加逻辑抢购火影专区的系统卡撸羊毛超过三次的用户几率再降低1/5
					if("1".equals(data.get("forder").toString())) {
						//查询当前用户是否已经撸过三次羊毛
						int yangnum = this.rushBuyClacMapper.getHuoYingNum(indexlist.get(i));
						if(yangnum >= 3) {
							logger.info("该用户已经撸过三次羊毛： userid:"+indexlist.get(i).get("userid"));
							insertNotice("抢购"+cardInfo.get("typename"), orderlist.get(i).get("title")+":"+data.get("calctime")+"-"+indexlist.get(i).get("nickname")+"该用户已经撸过三次羊毛：该用户抢购失败", Constants.FAILED);
							continue;
						}
					}
		...
		if (lastflag == 1) {
			int disnum = 0;
			//最后一次特殊处理海贼王手办分区的情况 优先让抢购的人吃掉订单
			if("8".equals(data.get("forder").toString()) && orderlist.size() > indexlist.size()) {
				Map<String, Object> orderMap = new HashMap<String, Object>();
				for(int i = indexlist.size(); i < orderlist.size(); i++) {
					//可分配用户用完或者待出售订单消耗完 及结束
					if (i == userlist.size()) {
						break;
					}
					//直接分配给未分配的用户
					orderMap.put("buyuserid", userlist.get(i).get("userid"));
					//分配待出售会员卡给用户的情况
					orderMap.put("orderid", orderlist.get(i).get("orderid"));
					//当抢购人员分配到自己正在出售的会员卡此等极端情况 直接不给他分配了 当做他没抢到
					if(userlist.get(i).get("userid").equals(orderlist.get(i).get("selluserid"))) {
						logger.info("当抢购时分配到到自己正在出售的会员卡，就不分配了 userid:"+userlist.get(i).get("userid"));
						insertNotice("抢购"+cardInfo.get("typename"), orderlist.get(i).get("title")+":"+data.get("calctime")+"-"+userlist.get(i).get("nickname")+"抢购时分配到到自己正在出售的会员卡：该用户抢购失败", Constants.FAILED);
						continue;
					}
					orderMap.put("status", 1);
					//根据买家等级和会员卡收益率和会员卡价格计算一个收益价格profitprice
					double cardprice = Double.parseDouble(orderlist.get(i).get("cardprice").toString());
					double yield = Double.parseDouble(cardInfo.get("yield").toString());
					double interesttimes = Double.parseDouble(userlist.get(i).get("interesttimes").toString());
					double profitprice = cardprice * yield * interesttimes / 100;
					orderMap.put("profitprice", profitprice);
					//根据抢购时间和出售所需天数计算一个到期时间duetime
					orderMap.put("commentstartdays", cardInfo.get("commentstartdays"));
					...
					Map<String, Object> rushToBuyMap = new HashMap<String, Object>();
					rushToBuyMap.put("cardid", cardInfo.get("cardid"));
					rushToBuyMap.put("userid", userlist.get(i).get("userid"));
					rushToBuyMap.put("aftstatus", 2);
					rushToBuyMap.put("befstatus", 1);
					this.rushBuyClacMapper.updateRushToBuy(rushToBuyMap);
					//用户扣除抢购所需XGO币
					...
					this.appUserCenterMapper.addXgoRecord(rushToBuyMap);
					//给买家提示通知
					insertUserNotice("抢购"+orderlist.get(i).get("title"), Constants.sysSmsTranslate3.replace("price", cardprice+"").replace("typename", orderlist.get(i).get("title").toString()), Constants.smsTranslate3.replace("typename", orderlist.get(i).get("title").toString()), userlist.get(i).get("userid").toString(), userlist.get(i).get("phone").toString());
					//给卖家提示通知
					insertUserNotice("出售"+orderlist.get(i).get("title"), Constants.sysSmsTranslate1.replace("price", cardprice+"").replace("typename", orderlist.get(i).get("title").toString()), Constants.smsTranslate1.replace("typename", orderlist.get(i).get("title").toString()), orderlist.get(i).get("selluserid").toString(), orderlist.get(i).get("phone").toString());
					//系统通知
					insertNotice("抢购成功", cardInfo.get("typename")+":"+data.get("calctime")+"恭喜用户"+userlist.get(i).get("nickname")+"抢购价值"+cardprice+"元的"+orderlist.get(i).get("title")+"成功", Constants.SUCCESS);
					allcount++;
					disnum ++;
				}
			}
			 ...
			 * 3、随机分配给系统用户回收订单
			 */
			if (orderlist.size() > (indexlist.size() + disnum)) {
				for(int i = (indexlist.size() + disnum); i < orderlist.size(); i++) {
					Map<String, Object> sysMap = new HashMap<String, Object>();
					sysMap.put("userid", orderlist.get(i).get("selluserid"));
					//查询所有系统用户
					sysUserList = this.rushBuyClacMapper.getSysUser(sysMap);
					//从系统用户中随机取一个用户回收手办
					int index = random.nextInt(sysUserList.size());
					Map<String, Object> orderMap = new HashMap<String, Object>();
