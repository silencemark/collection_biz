...
//	    cal.add(Calendar.DATE, -1);
	...
	public static String text2text(String text){
		SimpleDateFormat sdfL = new SimpleDateFormat("yyyy-MM-dd HH:mm");  
		try {
			Date date = sdfL.parse(text);
			sdfL = new SimpleDateFormat("MM-dd HH:mm");
			return sdfL.format(date);
		} catch (ParseException e) { 
			e.printStackTrace();
		}
...
//		  LOGGER.debug(calendar.getTime());
		//得到当前这个月
		//LOGGER.debug(calendar.get(Calendar.DAY_OF_WEEK)+1); 
	    //calendar.setTime(date);
	    //LOGGER.debug(calendar.getActualMaximum(Calendar.DAY_OF_MONTH));
//		calendar.set(Calendar.DAY_OF_WEEK, 1);
//		SimpleDateFormat format = new SimpleDateFormat("E");
//		LOGGER.debug("本月第一天是：" + format.format(calendar.getTime())); 
...
//		}
		...
		//LOGGER.debug(	DateUtil.changeTime(date, "x_y").getMonth());
		
	}
	
	/**
	 * 判断两个时间的大小 前者大于后者返回true 反之false
	 * @param d1
	 * @param d2
	 * @return
	 */
	...
	public static void main(String[] args) {
		...
		System.err.println(DateUtil.dateToWeek(new Date()));
	//	LOGGER.debug(DateUtil.forMM(new Date()));
//		DateUtil.getcalendar(new Date(),null);
//		DateUtil.getcalendar(DateUtil.changeTime(DateUtil.changeTime(DateUtil.changeTime(new Date(), "x_y"), "x_y"),"x_y") ,null);
	//	LOGGER.debug(DateUtil.compare("2015-07-18 23:12:21","2015-07-18 22:12:21"));
		//lastToweek(new Date());
		//Date currentDate = DateUtil.text("2015-07-19");
		//LOGGER.debug(DateUtil.only(new Date(), "1975-07-19"));
